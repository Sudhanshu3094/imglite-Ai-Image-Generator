...
const Login = () => {
    ...
    // → This disables scrolling on the entire page (<body>).
    // So, the user can’t scroll the background while this component is active (common in modals or overlays).

    //      When the component unmounts (is removed):
    // The function inside return runs (the cleanup).
        ...
        <div className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
            <motion.form
                ...
                {state !== 'Login' && <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-4'>
                    <img src={assets.user_icon} alt="" width={22} />
                ...
                <button className='bg-red-600 w-full text-white py-2 rounded-full'>{state == 'Login' ? 'login' : 'create account'}</button>
...
//z-10 → ensures it appears above elements with a lower z-index (like default z-0).
